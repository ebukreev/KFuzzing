...
classParameter
    : modifierList? (VAL | VAR)? simpleIdentifier COLON type_i (ASSIGNMENT expression)?
    ;
...
getter
    : modifierList? GETTER
    | modifierList? GETTER NL* LPAREN RPAREN (NL* COLON NL* type_i)? NL* (block | ASSIGNMENT NL* expression)
    ;
...
typeAlias
    : modifierList? TYPE_ALIAS NL* simpleIdentifier (NL* typeParameters)? NL* ASSIGNMENT NL* type_i
    ;
...
typeParameter
    : modifierList? NL* simpleIdentifier (NL* COLON NL* type_i)?
    ;
...
functionType
    : (functionTypeReceiver NL* DOT NL*)? functionTypeParameters  NL* ARROW (NL* type_i)
    ;
...
functionTypeParameters
    : LPAREN (parameter | type_i)? (COMMA (parameter | type_i))* RPAREN
    ;
...
typeConstraint
    : annotations* simpleIdentifier NL* COLON NL* type_i
    ;
...
namedInfix
    : elvisExpression ((inOperator NL* elvisExpression)+ | (isOperator NL* type_i))?
    ;
...
typeProjection
    : typeProjectionModifierList? type_i | MULT
    ;
...
lambdaParameter
    : variableDeclaration
    | multiVariableDeclaration (NL* COLON NL* type_i)?
    ;
...
superExpression
    : SUPER (LANGLE NL* type_i NL* RANGLE)? LabelReference?
    ;
