...
//taken from https://github.com/ItsLastDay/KotlinFuzzer/blob/master/parser/Kotlin.g4
grammar Kotlin;
...
typeAlias
  : modifiers KW_TYPEALIAS simpleName typeParameters? '=' type_
  ;
...
function
  : modifiers KW_FUN
      ...
      (receiverType=type_ '.')?
      simpleName
      typeParameters? valueParameters (':' returnType=type_)?
      typeConstraints
      functionBody?
  ;
...
getter
  : modifiers KW_GET
  | modifiers KW_GET '(' ')' (':' type_)? functionBody
  ;
...
functionType
  : (funcReceiverType '.')? '(' functiontype_parameter_comma? ')' '->' type_?
  ;
...
expression
  : expression SEMI* (typeOperation | isOperation) SEMI* type_  # expTypeOp
...
atomicExpression
  ...
  | KW_SUPER ('<' type_ '>')? labelReference?  # atomExpSuper
...
lambdaParameter
  : variableDeclarationEntry
  | multipleVariableDeclarations (':' type_)?
  ;
...
type_comma: SEMI* type_ SEMI* (',' SEMI* type_ SEMI*)* ;
