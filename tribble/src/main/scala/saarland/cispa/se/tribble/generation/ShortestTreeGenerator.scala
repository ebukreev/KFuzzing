...
class ShortestTreeGenerator(regexGenerator: RegexGenerator, random: Random, closeOffChoice: Int) extends TreeGenerator {
  require(closeOffChoice > 0, s"Close-off choice hint must be positive! ($closeOffChoice given)")

  def gen(rule: DerivationRule, parent: Option[DNode], currentDepth: Int)(implicit grammar: GrammarRepr): DTree = generate(rule, parent)(grammar, mutable.Map.empty.withDefaultValue(closeOffChoice - 1))

  def generate(rule: DerivationRule, parent: Option[DNode])(implicit grammar: GrammarRepr, closeOffs: mutable.Map[DerivationRule, Int]): DTree = rule match {
